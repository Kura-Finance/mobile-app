...
export default function InsightsModal({ isOpen, onClose, transactions }: InsightsModalProps) {
    ...
    >
      <View style={{ flex: 1, backgroundColor: colors.background }}>
        <View style={[styles(colors).header, { paddingTop: insets.top + 12 }]}>
          <Text style={styles(colors).headerTitle}>{t('trackfi.banking.insights')}</Text>
        ...
        >
          <Text style={styles(colors).subtitle}>{t('trackfi.banking.insightsSubtitle')}</Text>
          ...
          <View style={styles(colors).summaryCard}>
            <View style={styles(colors).summaryItem}>
              ...
              <CurrencyDisplay value={totalIncome} fontSize={18} color={colors.success} style={{ fontWeight: '700' }} />
            </View>
            <View style={styles(colors).summaryDivider} />
            <View style={styles(colors).summaryItem}>
              <Text style={styles(colors).summaryLabel}>{t('dashboard.spending')}</Text>
              <CurrencyDisplay value={totalSpending} fontSize={18} color={colors.danger} style={{ fontWeight: '700' }} />
            </View>
            <View style={styles(colors).summaryDivider} />
            <View style={styles(colors).summaryItem}>
          ...
          {!hasData && (
            <View style={{ paddingVertical: 48, alignItems: 'center' }}>
              <Text style={{ color: colors.textMuted, fontSize: 14 }}>{t('trackfi.banking.noInsightsData')}</Text>
            </View>
          )}
