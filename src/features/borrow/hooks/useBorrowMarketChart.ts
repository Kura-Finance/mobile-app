...
export function useBorrowMarketChart(
  ...
  useEffect(() => {
    ...
    const key = `${marketId.toLowerCase()}:${timeframe}`;
    ...
    void getMarketBorrowApyHistory(marketId, { startTimestamp, endTimestamp, interval })
      .then((series) => {
